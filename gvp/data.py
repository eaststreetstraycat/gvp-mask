...
class CATHDataset:
    ...
    def __init__(self, path, splits_path):
        ...
        train_list, val_list, test_list = dataset_splits['train'], \
            dataset_splits['val'], dataset_splits['test']
...
class ProteinGraphDataset(data.Dataset):
    ...
    def __init__(self, data_list, 
                 top_k,maskpart,num_positional_embeddings=16, num_rbf=16, device="cpu"):
        
        super(ProteinGraphDataset, self).__init__()
        
        self.data_list = data_list
        self.top_k = top_k
        self.maskpart = maskpart
        ...
        self.letter_to_num = {'C': 4, 'D': 3, 'S': 15, 'Q': 5, 'K': 11, 'I': 9,
                       ...
                       'N': 2, 'Y': 18, 'M': 12, 'X':20,'U':21,'O':22,'B':23}
    ...
    def _featurize_as_graph(self, protein):
        ...
        with torch.no_grad():
            ...
            onehot = self._onehot(seq,self.maskpart)
            
            orientations = self._orientations(X_ca)
            sidechains = self._sidechains(coords)
             
            
            node_s = torch.cat([dihedrals,onehot],dim=1)
            #node_s = dihedrals
	...
	#add new ONE-HOT embeding
    def _onehot(self,labels,mask):
        ...
        for i in range(len(labels)):
            if random.random() < mask:
                if random.random() < 0.8:
                    ...
                    labels[i] = random.randint(0,19)
                else:
                    labels[i] = labels[i]
            else:
                labels[i] = labels[i]
        
        one_hot_label = np.array([[int(i == int(labels[j])) for i in range(25)] for j in range(len(labels))])
        py = torch.from_numpy(one_hot_label)
