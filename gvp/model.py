...
def _esm_forward(model,alphabet,seq):
    for sequence in range(len(seq)):
        model = model.to(device)
        data = [('protein',seq[sequence]),]
        ...
        esm_vec = token_representations[1:-1]
        if sequence == 0:
            vec = esm_vec
        else:
            vec = torch.cat([vec,esm_vec],dim=0)
    #print(vec.shape)
    return vec
...
class MQAModel(nn.Module):
    ...
    def __init__(self, node_in_dim, node_h_dim, 
        ...
        self.esmmodel,self.esmalphabet = esm.pretrained.esm1b_t33_650M_UR50S()
    ...
    def forward(self, h_V, edge_index, h_E, seq, batch=None):      
        ...
        node_embeddings = _esm_forward(self.esmmodel,self.esmalphabet,seq)
        node_emdeddings = node_embeddings.to(device)
