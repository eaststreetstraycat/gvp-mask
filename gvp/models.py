...
class CPDModel(torch.nn.Module):
    ...
    def __init__(self, node_in_dim, node_h_dim, 
        ...
        self.W_s = nn.Embedding(24, 24)
        edge_h_dim = (edge_h_dim[0] + 24, edge_h_dim[1])
        ...
        self.W_out = GVP(node_h_dim, (24, 0), activations=(None, None))
    ...
    def sample(self, h_V, edge_index, h_E, n_samples, temperature=0.1):
        ...
        with torch.no_grad():
            ...
            h_S = torch.zeros(n_samples * L, 24, device=device)
...
class MQAModel(nn.Module):
    ...
    def __init__(self, node_in_dim, node_h_dim, 
                 edge_in_dim, edge_h_dim,
                 seq_in=False, num_layers=3, drop_rate=0.1,residual=True):
...
class gvpfinetunemodel(nn.Module):
    def __init__(self,node_in_dim,node_h_dim,edge_in_dim,edge_h_dim,drop_rate=0.1):
        super(gvpfinetunemodel,self).__init__()
        device = "cuda" if torch.cuda.is_available() else "cpu"
        self.model = CPDModel((31,3),(100,16),(32,1),(32,1)).to(device)
        self.model.load_state_dict(torch.load('/home/huchao/work/gvp-pytorch/models/1659757581_96.pt'))
        self.dense = nn.Sequential(nn.Linear(24,40),nn.ReLU(inplace=True),nn.Dropout(p=drop_rate),nn.Linear(40,1))
    def forward(self,h_V,edge_index,h_E,seq,batch=None):
        #with torch.no_grad():
        #device = "cuda" if torch.cuda.is_available() else "cpu"
        #model = CPDModel((31,3),(100,16),(32,1),(32,1)).to(device)
        #model.load_state_dict(torch.load('/home/huchao/work/gvp-pytorch/models/1659757581_96.pt'))
        x =self.model(h_V,edge_index,h_E,seq)
             #for param in model.parameters():
             #    param.requires_grad = False
    
        if batch is None: out = x.mean(dim=0,keepdims=True)
        else: out = scatter_mean(x,batch,dim=0)
        return self.dense(out).squeeze(-1) + 0.5 
