...
parser.add_argument('--top-k', metavar='N', type=int, default=30,
                    help='edges in the k-nearest neighbors graph')
parser.add_argument('--mask', metavar='N', type=float, default=0.15,
                    help='mask prediction')
...
def main():
    ...
    model = gvp.models.CPDModel((31, 3), node_dim, (32, 1), edge_dim).to(device)
    
    print("Loading CATH dataset")
    cath = gvp.data.CATHDataset(path="data/cath/chain4.3_set.jsonl",
                                splits_path="data/cath/chain4.3_set_split.json")    
    
    trainset, valset, testset = map(gvp.data.ProteinGraphDataset,
                                    (cath.train, cath.val, cath.test),(args.top_k,args.top_k,args.top_k),(args.mask,args.mask,args.mask))
    
    if args.test_r or args.test_p:
        ts50set = gvp.data.ProteinGraphDataset(json.load(open(args.ts50)),args.top_k,args.mask)
...
def train(model, trainset, valset, testset):
    ...
    for epoch in range(args.epochs):
        ...
        with open('/home/huchao/work/gvp-pytorch/best.txt','w',encoding='utf-8') as f:
...
def test_recovery(model, dataset):
    recovery = []
    result = open('/home/huchao/work/gvp-pytorch/result.txt','a',encoding='utf8')
    for protein in tqdm.tqdm(dataset):
        ...
        #result.write(str(recovery_)+'/n')
        #result.flush()
    recovery = np.median(recovery)
    print(f'TEST recovery: {recovery:.4f}')
    
    result.writelines(str(recovery) + '/n')
    result.flush()
def loop(model, dataloader, optimizer=None):
    ...
    for batch in t:
        ...
        confusion += confusion_matrix(true, pred, labels=range(24))
